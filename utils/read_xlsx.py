...
class XlsxReader:
    ...
    def get_xlsx_flights_data():
        ...
        for i in range(1, sheet.nrows):
            search_flights_data = SearchFlightsData(sheet.cell(i, 0).value, sheet.cell(i, 1).value,
                                                    sheet.cell(i, 2).value, int(sheet.cell(i, 3).value),
                                                    int(sheet.cell(i, 4).value), sheet.cell(i, 5).value,
                                                    int(sheet.cell(i, 6).value), int(sheet.cell(i, 7).value),
                                                    sheet.cell(i, 8).value, int(sheet.cell(i, 9).value),
                                                    int(sheet.cell(i, 10).value), int(sheet.cell(i, 11).value),
